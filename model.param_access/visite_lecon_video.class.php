...
class visite_lecon_video
{
    ...
    public static function ajouter($idLecon, $idCours, $iduti, $inscri)
    {
        ...
        $editeur = crs_lecon_video::getAuteur($idLecon)->fetch();


        if ($_SESSION['idUtilisateur'] != $editeur['idUtilisateur']) {

            if (self::estvue($idLecon, $inscri, $idutil) == false) {
                $req = con()->prepare('INSERT INTO visite_lecon_video (idLeconVideo,idCours,idUtilisateur,idInscription) VALUES (?,?,?,?)');
    ...
    {
        $vue = con()->query('SELECT idVisiteLeconVideo FROM `visite_lecon_video` as vlc WHERE vlc.idLeconVideo=' . $idLecon . ' AND vlc.idUtilisateur=' . $idUti . ' AND vlc.idInscription=' . $inscri);
        if ($vue->fetch() != null) {
            return true;
        } else {
            return false;
