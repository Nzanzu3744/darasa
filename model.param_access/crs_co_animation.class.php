...
class crs_co_animation
{
    ...
    //     $req=self::$con->exec("UPDATE `crs_co_animation` SET `idUtilisateur`=".$idUtilisateur.",`idCours`=".$idCours.", idClasse=".$idClasse.", `actif`=".$idActif." WHERE idCoAnimation`=".$idCoAnimation);
    // }
    ...
    public static function liste_co_animateur_crs($idCours,$idEcole)
    {
        return  self::$con->query('SELECT * FROM crs_co_animation as coa  INNER JOIN param_utilisateur as ut ON coa.idUtilisateur=ut.idUtilisateur  INNER JOIN org_classe as cls ON coa.idClasse=cls.idClasse WHERE coa.idCours=' . $idCours.' AND cls.idEcole='.$idEcole);
    }
