...
class crs_cours {
    ...
    public function selectionnerByUtCls($idUt,$cls){
        return  self::$con->query('SELECT * FROM `crs_cours`  as crs INNER JOIN org_affectation as af ON crs.idAffectation=af.idAffectation INNER JOIN param_utilisateur as ut ON ut.idUtilisateur=af.idUtilisateur WHERE ut.idUtilisateur='.$idUt.' AND af.idClasse='.$cls );
    }
    
     public function selectionnerCrsEleveByCls($cls){
        return  self::$con->query('SELECT * FROM `crs_cours`  as crs  INNER JOIN org_affectation as af ON crs.idAffectation=af.idAffectation INNER JOIN param_utilisateur as uti ON uti.idUtilisateur=af.idUtilisateur  INNER JOIN org_classe as cls ON cls.idClasse=af.idClasse INNER JOIN org_inscription as ins ON cls.idClasse=ins.idClasse WHERE ins.idClasse='.$cls );
    }
