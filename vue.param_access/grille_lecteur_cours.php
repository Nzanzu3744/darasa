    ...
    <table class="table table-bordered table-striped table-condensed">
        ...
        <tbody>
                <?php
                    ...
                    $grd_tour_vis= $grd_tour_vis ->visiLecteurAll();
                    $cpt=0;
                     foreach($grd_tour_vis  as $sel_grd_tour_vis){
                        $cpt++;
                        echo '<tr>';
                            echo '<td style="color:red">'.$cpt.'</td><td>'.$sel_grd_tour_vis['nomUtilisateur'].' '.$sel_grd_tour_vis['postnomUtilisateur'].' '.$sel_grd_tour_vis['prenomUtilisateur'].'</td>';
                            $tour_visite_lc_pres =new visite_lecon();
                            $tour_visite_lc_pres = $tour_visite_lc_pres->vueVisiLecteurUtilCours($sel_grd_tour_vis['idUtilisateur'],$sel_grd_tour_vis['idCours']);
                            $tourRel=0;
                            foreach($tour_visite_lc_pres as $sel_tour_visite_lc_pres){
                                ...
                                for($i=0; $i<=$key;$i++){
                                    ...
                                    if($i==$key){
                                        echo '<td style="color:green; font-size:18px"><span class="glyphicon glyphicon-ok"></span></td>';
                                    }else{
         ...
         <?php
                ...
                // foreach($lc as $selLc){
                //     $grd_tour_vis =new visite_lecon();
                //     $grd_tour_vis= $grd_tour_vis ->vuesGpUtilLectourPreve($selLc['idLecon']);
                //      foreach($grd_tour_vis  as $sel_grd_tour_vis){
                //         echo '<tr>';
                //             echo '<td>'.$sel_grd_tour_vis['nomUtilisateur'].'</td>';
                //             $tour_visite_lc_pres =new visite_lecon();
                //             $tour_visite_lc_pres = $tour_visite_lc_pres->vues($selLc['idLecon']);
                //             foreach($tour_visite_lc_pres as $sel_tour_visite_lc_pres){
                //                 // echo '<td>vst'.$sel_tour_visite_lc_pres['idUtilisateur'].'</td>';
                //         }
                //         echo '</tr>';
                //         }
