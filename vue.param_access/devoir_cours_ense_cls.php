                  ...
                  <button data-toggle="dropdown" style=""><b class="caret ppull-right"></b></button>
                       ...
                       $sel_C=$sel_C->selectionnerByCours($seldv['idCours'])->fetch();
                        ?>
                              ...
                              <li data-toggle="modal" href="#inscri"><a href="#" onclick="Orientation('control.param_access/ctr_questionnaire.php?Correctdevoirs_ense=tue&maClasse=<?=$_GET['maClasse']?>$idAnneeSco=<?=$_GET['idAnneeSco']?>&idClasse=<?=$_GET['idClasse']?>&cours=<?=$sel_C['cours']?>&idcrs=<?=$sel_C['idCours']?>&iddv=<?=$seldv['idDevoir']?>','#editLeco','')">Correction devoirs</a></i></li>
                              
                              <li class="divider"></li>
                              <li>
                                 <a onclick="Orientation('control.param_access/ctr_devoirs.php?supDev=true&fct=ense&idDev=<?=$seldv['idDevoir']?>maClasse=<?=$_GET['maClasse']?>&cours=<?=strtoupper($sel_C['cours'])?>&idCours=<?=$sel_C['idCours']?>&idAnneeSco=<?=$_GET['idAnneeSco']?>&idClasse=<?=$_GET['idClasse']?>','#leconsgauche','')" style="color: red" >Supprimer</a>
                              </li>
                        </ul>
                     </z>                
               </td>
