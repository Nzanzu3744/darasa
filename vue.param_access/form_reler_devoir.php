...
<div class="form-inline well" style="width:100%x; height:300px ; font-size:12px; background:white">
    ...
    <div style="margin-top:100px">
        <input id="enrg" type="button" onclick='Orientation("../control.param_access/ctr_devoirs.php?RelierDevoir=true&idAnneeSco=<?= $_GET["idAnneeSco"] ?>&idCls=<?= $_GET["idCls"] ?>&maClasse=<?= $_GET["maClasse"] ?>&idCours=<?= $_GET["idCours"] ?>&idPrepaCours=<?= $_GET["idPrepaCours"] ?>&idDevoir=<?= $_GET["idDevoir"] ?>&dateRm="+$("#dateRm").val()+"&idPr02="+$("#idPr02").val(),"#leconsgauche")' class="btn btn-success pull-left col-sm-4" value="OK" />
