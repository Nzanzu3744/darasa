    ...
    <table class="table table-bordered table-striped table-condensed">
                    ...
                    ?>
                        ...
                        <th>
                            <?php 
                                ...
                                $nremis = $nremis->remis($seldv['idDevoir']);
                                $nbrm=0;
        ...
        <tbody>
                <?php
                    ...
                    $grd_tour_remis = $grd_tour_remis ->RemisEleveAll();
                    $cpt=0;
                     foreach($grd_tour_remis  as $sel_grd_tour_remis){
                        $cpt++;
                        echo '<tr>';
                            echo '<td style="color:red">'.$cpt.'</td><td>'.$sel_grd_tour_remis['idUtilisateur'].' '.$sel_grd_tour_remis['nomUtilisateur'].' '.$sel_grd_tour_remis['postnomUtilisateur'].' '.$sel_grd_tour_remis['prenomUtilisateur'].'</td>';
                            $grd_tour_remis_encours =new suivie_remise_devoirs();
                            $grd_tour_remis_encours = $grd_tour_remis_encours->remiseDevEleveCours($sel_grd_tour_remis['idUtilisateur'],$sel_grd_tour_remis['idCours']);
                            $tourRel=0;
                            foreach($grd_tour_remis_encours as $sel_grd_tour_remis_encours){
                                ...
                                for($i=0; $i<=$key;$i++){
                                    ...
                                    if($i==$key){
                                        echo '<td style="color:green; font-size:18px"><span class="glyphicon glyphicon-ok">'.$sel_grd_tour_remis_encours['idUtilisateur'].'</span></td>';
                                    }else{
