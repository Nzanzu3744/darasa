...
<input id="btn_enreg_lecon" value="ENREGISTRER" type="button" class="btn btn-success pull-left  col-sm-2" style="padding:6px" onclick="Orientation('../control.param_access/ctr_lecon.php?maClasse=<?=$_GET['maClasse']?>&tlecon='+$('#tlecon').val()+'&idlc='+$('#idlc').val()+'&cours=<?=$_GET['cours']?>&idcrs=<?=$_GET['idcrs']?>&ajouterL=true','#editLeco','#lecon');Orientation('../control.param_access/ctr_lecon.php?leconsgauche_ense&idCours=<?=$_GET['idcrs']?>','#leconsgauche','');"  ></input> 
